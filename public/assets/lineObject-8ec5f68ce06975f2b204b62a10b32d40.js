...
function LineDragger(label, color, loc) {
...
var p = createjs.extend(LineDragger, createjs.Container);
...
p.doodle = function() {
	...
	this.addChild(line);
	line.graphics.setStrokeStyle(4).beginStroke(this.color);
...
p.handlePressMove = function(evt) {
	  ...
	  if (clicked.near(real, delta))
	  {
		 ...
		 this.drawline(evt, this.point0, this.point1, 4, this.color);
	  }
...
p.drawline = function(evt, Point0, Point1, strokeStyle, colorStyle) {
...
window.LineDragger = createjs.promote(LineDragger, "Container");
}());
