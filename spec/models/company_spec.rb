...
describe Company do
  it "has a valid name" do
     record = FactoryGirl.create(:company,:name,:license_rand,:portal)
     expect(record).to be_valid
  end
  it "is invalid without a name" do
     expect{FactoryGirl.create(:company,:noname,:license_rand,:portal)}.to raise_error(ActiveRecord::RecordInvalid)
  end

  it "is invalid without a portal" do
     expect{FactoryGirl.create(:company,:name,:license_rand,:noportal)}.to raise_error(ActiveRecord::RecordInvalid)
  end
  ...
  it "cannot be destroyed with divisions" do
    record = FactoryGirl.create(:company,:name,:license_rand,:portal,:with_divisions)
    expect{record.destroy!}.to raise_error(ActiveRecord::RecordNotDestroyed)
  end

  it "cannot have a duplicate" do
    record = FactoryGirl.create(:company, :namefixed, :license_rand,:portal,:with_divisions)
