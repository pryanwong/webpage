...
describe DrawingsController, :type => :controller do
    ...
    let(:company4) { double("company4", update_attributes: {name: "batman", licenses: "3", portal: "portal"}, errors: {}, save: true, :id => 1)}
    ...
    describe "GET 'edit'" do
      ...
      it "renders the portal page because user was logged in as admin" do
         allow(user).to receive_messages(:id => 1, :role=>3, :admin? => true, :moderator? => false, :user? => false, :email => "sf@test.com", :company_id => 1, :company => company4)
         allow(drawing).to receive_messages(:drawing=>"blah")
         ...
         allow(company4).to receive_messages(:portal => "Demo_Industrial")
         allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)
         Drawing.stub_chain(:moderator_access, :pluck, :include?) {true}
         login(user.email)
         get :edit, :company_id => 1, :user_id => 1, :id => 1
         expect(response).to render_template("layouts/editlayout")
         expect(response).to render_template("drawings/portals/Demo_Industrial")
         expect(assigns(:editdetails)).to eq(true)
      end

      it "renders the portal page because user was logged in as moderator" do
        allow(user).to receive_messages(:id => 1, :role=>2, :admin? => false, :moderator? => true, :user? => false, :email => "sf@test.com", :company_id => 1, :company => company4)
        drawing2 = double("drawing", :company_id => 1, :drawing=>"blahblah", :id => 1, :user_id => 1, :pluck =>[1] )
        allow(User).to receive(:where).and_return([user])
        allow(User).to receive(:find).and_return(user)
        ...
        allow(Company).to receive(:find).and_return(company4)
        allow(Drawing).to receive(:exists?).and_return(true)
        allow(Drawing).to receive(:where).and_return([drawing2])
        allow(Drawing).to receive(:find).and_return(drawing2)
        allow(Drawing).to receive_message_chain(:moderator_access, :pluck, :include? ).and_return(true)
        allow(drawing2).to receive(:pluck).and_return([1])
        allow(company4).to receive_messages(:portal => "Demo_Industrial")
        allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)
        login(user.email)
        get :edit, :company_id => 1, :user_id => 1, :id => 1
      ...
      it "renders the portal page because user was logged in as moderator, with drawing blank" do
         drawing2 = double("drawing", :company_id => 1, :drawing=>"",:id => 1, :user_id => 1, :pluck =>1 )
         allow(user).to receive_messages(:id => 1, :role=>2, :admin? => false, :moderator? => true, :user? => false, :email => "sf@test.com", :company_id => 1, :company => company1)
         allow(User).to receive(:where).and_return([user])
         allow(User).to receive(:find).and_return(user)
         allow(drawing2).to receive_messages(:drawing=>"blahblah")
         allow(Drawing).to receive(:exists?).and_return(true)
         allow(Drawing).to receive(:find).and_return(drawing2)
      ...
      it "renders the index page because user was logged in as user" do
         drawing2 = double("drawing", :company_id => 1, :drawing=>"", :drawing= => "", :id => 1, :user_id => 1, :pluck =>1 )
         allow(user).to receive_messages(:id => 1, :role=>1, :admin? => false, :moderator? => false, :user? => true, :email => "sf@test.com", :company_id => 1, :company => company1)
         allow(User).to receive(:where).and_return([user])
         allow(User).to receive(:find).and_return(user)
         allow(drawing2).to receive_messages(:drawing=>"blahblah")
         allow(Drawing).to receive(:exists?).and_return(true)
    ...
    describe "GET 'editdrawingdetails'" do
      it "renders the editdrawingdetails page" do
        allow(user).to receive_messages(:id => 1, :role=>1, :admin? => false, :divisions => divisions, :moderator? => false, :user? => true, :email => "sf@test.com", :company_id => 1, :company => company1)
        allow(User).to receive(:where).and_return([user])
        allow(User).to receive(:find).and_return(user)
    ...
    describe "GET 'updatedrawingdetails'" do
      it "renders the updatedrawingdetails page" do
        ...
        expect(response).to redirect_to(company_user_path(company1.id, user.id))
      end
